...
import chess/search/search_state2.{type SearchState}
...
pub fn search_loop(
  ...
  use best_evaluation: Evaluation <- interruptable.do_checkpoint({
    use <- search_state2.check_interrupt
    let game_hash = game.hash(game)
    use cached_evaluation <- interruptable.do(
      interruptable.into(search_state2.transposition_get(game_hash)),
    )
  ...
  use <- state.discard({
    ...
    use info <- state.do(
      search_state2.stats_to_string(_, now)
      |> state.select,
    )
    io.print_error(info)
  ...
  case opts.max_depth {
    ...
    _ ->
      case best_evaluation.score {
        ...
        xint.PosInf | xint.NegInf -> state.return(Ok(best_evaluation))
        _ -> search_loop(game, current_depth + 1, opts, game_history)
      }
  }
}
...
fn negamax_alphabeta_failsoft(
  ...
  use cached_evaluation <- state.do({
    ...
    search_state2.transposition_get(game_hash)
    |> state.map(fn(x) {
  ...
  use <- state.discard(search_state2.stats_increment_nodes_searched())
  use <- state.discard(case depth >= tt_min_leaf_distance {
    True -> {
      use <- state.discard(
        search_state2.transposition_insert(game_hash, #(depth, evaluation)),
      )
      use <- state.discard(search_state2.transposition_prune(
        when: search_state2.LargerThan(max_tt_size),
        do: search_state2.ByRecency(max_tt_recency),
      ))
    ...
    let finish = fn(e: Evaluation, alpha, beta) {
      ...
      case xint.gte(e.score, beta) {
        True -> {
          ...
          use <- interruptable.discard(
            case move_context.capture |> option.is_none {
              True -> {
                ...
                interruptable.into(search_state2.history_update(
                  #(to, piece),
                  depth * depth,
                ))
              }
          ...
          use <- interruptable.discard(
            interruptable.into(search_state2.stats_add_beta_cutoffs(
              depth,
              nmoves - idx + 1,
            )),
          )
...
fn get_pv_move(game: game.Game, depth: evaluation.Depth, alpha, beta, history) {
  use cached_entry <- state.do(search_state2.transposition_get(game.hash(game)))
...
fn sorted_moves(
  ...
  use compare_quiet_history <- interruptable.map({
    ...
    Ok(compare_quiet_history(search_state.history))
  })
